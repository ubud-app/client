module.exports = function (grunt) {
	...
	const paths = require(__dirname + '/app/index.js');
	const configuration = {
		build: {
	...
	grunt.initConfig({
		...
		replace: {
			...
			jsConfiguration: {
				options: {
					patterns: [
						{
							match: 'CONFIGURATION_PRODUCTION',
							replacement: configuration.production || ''
						},
						{
		...
		uglify: {
			options: {
				sourceMap: true,
				output: {
					preamble: paths.options.js.banner.development
				},
				compress: {
					dead_code: true,
					drop_debugger: false
			...
			app: {
				options: {
					sourceMapIn: paths.static + '/main.js.map'
				},
				src: paths.static + '/main.js',
				dest: paths.static + '/main.min.js'
			}
		},
		watch: {
			options: {
